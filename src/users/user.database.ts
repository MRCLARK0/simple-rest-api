...
export const create = async (userData: UnitUser): Promise<UnitUser | null> => {
  ...
  const hashedPassword = await bcrpyt.hash(userData.password, salt);
...
export const comparePassword = async (
  ...
  const decryptPassword = await bcrpyt.compare(
    supplied_password,
    user!.password
  );
...
export const update = async (
  ...
  if (updateValues.password) {
    const salt = await bcrpyt.genSalt(10);
    const newPass = await bcrpyt.hash(updateValues.password, salt);
